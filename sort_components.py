...
capacitors_re = re.compile(r"^(C\d+)?\s(\d+[uUnNpP]\d*)\s?(.*)?")
capacitor_value_re = re.compile(r"\D*\d*\s?(\d+[uUnNpP]\d*)\s?(.*)?")
...
def process_simple(line, rows, totals, line_regex, value_regex, value_prefix, separator):
	...
	if value_regex_result in totals:
		totals[value_regex_result]["total"] +=  1
	else:
...
def output_totals(totals, separator, filename, parts_url):
	list = []
	if parts_url == "":
		list = [f"{key}{separator}{value["total"]}" for key, value in totals.items()]
	else:
		list = [f"{key}{separator}{value["total"]}{separator}{parts_url.replace('%s',value["name"])}" for key, value in totals.items()]
	list.sort()
	out = ""
	for item in list:
		out += f"{item}\n"
...
def main(in_file, parts_file, totals_file, projects_file, parts_url, separator):
	parts = []
	totals = {"UNPROCESSED": {"name": "Unprocessed", "total": 0}}
	projects = []
	f = open(in_file, "r")
	str_in = f.read()
...
if __name__ == "__main__":
	...
	parser.add_argument(
		...
		default='=HYPERLINK("https://www.taydaelectronics.com/catalogsearch/result/?q=%s","Tayda")',
	)
